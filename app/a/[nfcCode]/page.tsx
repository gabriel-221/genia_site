...
export default async function AnimalPage({
  ...
  // Pedigree + filhos em paralelo
  const [{ data: pedigree }, { data: filhosRaw }] = await Promise.all([
  ...
  if (pedigree?.pai_id) {
    const { data } = await supabase
      .from('genia_animal')
      .select('id, nome, especie, sexo')
      .eq('id', pedigree.pai_id)
      .single()
    pai = data
  }
  if (pedigree?.mae_id) {
  ...
  if (animal.sexo === 'femea') {
    ...
    if (eventos?.length) {
      ...
      const inseminacoes = eventos.filter(
        (e: EventoReprodutivo) => e.tipo === 'inseminacao',
      )
      const confirmadas = inseminacoes.filter((e: EventoReprodutivo) => e.gestacao_confirmada)
